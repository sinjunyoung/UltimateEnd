...
namespace UltimateEnd.Services
{
    public class PlatformInfoService
    {
        ...
        private static PlatformDatabase? _database;
        private static readonly object _lock = new();
        private static Dictionary<string, PlatformInfo>? _aliasMap;
        private static Dictionary<string, string>? _shortestAliasCache;
        ...
        private static string GetConfigFilePath()
        {
            ...
            if (provider != null)
                return Path.Combine(provider.GetAppBaseFolder(), PlatformInfoFileName);

            return Path.Combine(AppContext.BaseDirectory, PlatformInfoFileName);
        }

        public static PlatformDatabase LoadDatabase()
        {
            if (_database != null)
                return _database;
            ...
            lock (_lock)
            {
                ...
                try
                {
                    ...
                    _database = JsonSerializer.Deserialize<PlatformDatabase>(json, options);
        ...
        {
            _aliasMap = new Dictionary<string, PlatformInfo>(StringComparer.OrdinalIgnoreCase);
            _shortestAliasCache = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);

            if (_database?.Platforms == null)
                return;

            foreach (var platform in _database.Platforms)
            {
                _aliasMap[platform.Id] = platform;

                foreach (var alias in platform.Aliases)
                    _aliasMap[alias] = platform;

                var allNames = new List<string> { platform.Id };
                allNames.AddRange(platform.Aliases);
                var shortest = allNames.OrderBy(n => n.Length).First();

                _shortestAliasCache[platform.Id] = shortest;

                foreach (var alias in platform.Aliases)
                    _shortestAliasCache[alias] = shortest;
            }
        }

        public static string GetPlatformDisplayName(string platformIdOrAlias)
        {
            if (string.IsNullOrWhiteSpace(platformIdOrAlias))
                return platformIdOrAlias;

            if (_aliasMap == null)
                LoadDatabase();

            var normalized = platformIdOrAlias.Trim().Replace(" ", string.Empty).ToLowerInvariant();

            if (_aliasMap!.TryGetValue(normalized, out var platform))
                return platform.DisplayName;

            return platformIdOrAlias.Trim();
        }

        public static string NormalizePlatformId(string platformName)
        {
            ...
            var normalized = platformName.Trim().Replace(" ", string.Empty).ToLowerInvariant();
        ...
        public static string ExtractPlatformIdFromFolderName(string folderName)
        {
            ...
            var inputNormalized = folderName.Trim().Replace(" ", string.Empty).ToLowerInvariant();
            ...
            foreach (var entry in _aliasMap!)
            {
                ...
                if (inputNormalized.StartsWith(alias))
                {
            ...
            if (bestMatchPlatform != null)
                return bestMatchPlatform.Id;

            return inputNormalized;
        }

        public static HashSet<string> GetValidExtensions(string platformId)
        {
            if (_aliasMap == null)
                LoadDatabase();
            ...
            if (_aliasMap!.TryGetValue(normalized, out var platform))
                return new HashSet<string>(platform.Extensions, StringComparer.OrdinalIgnoreCase);

            return [".zip", ".iso", ".chd"];
        }

        public static PlatformInfo? GetPlatformInfo(string platformIdOrAlias)
        {
            if (string.IsNullOrWhiteSpace(platformIdOrAlias))
                return null;
        ...
        public static ScreenScraperSystemId GetScreenScraperSystemId(string platformIdOrAlias)
        {
            if (string.IsNullOrWhiteSpace(platformIdOrAlias))
                return ScreenScraperSystemId.NotSupported;
            ...
            if (_aliasMap!.TryGetValue(normalized, out var platform))
                return platform.ScreenScraperSystemId;

            return ScreenScraperSystemId.NotSupported;
        }

        public static string GetShortestAlias(string platformIdOrAlias)
        {
            if (string.IsNullOrWhiteSpace(platformIdOrAlias))
                return platformIdOrAlias;

            if (_shortestAliasCache == null)
                LoadDatabase();

            var normalized = platformIdOrAlias.Trim().Replace(" ", string.Empty).ToLowerInvariant();

            if (_shortestAliasCache!.TryGetValue(normalized, out var shortest))
                return shortest;
        ...
        private static PlatformDatabase CreateDefaultDatabase()
        {
            ...
            db.Platforms.Add(new PlatformInfo { Id = "mastersystem", DisplayName = "Sega MasterSystem", Aliases = ["mastersystem"], Extensions = [".68k", ".bin", ".chd", ".cue", ".iso", ".m3u", ".gg", ".gen", ".md", ".mdx", ".rom", ".sg", ".sgd", ".smd", ".sms", ".zip"], ScreenScraperSystemId = ScreenScraperSystemId.SegaMasterSystem });
            ...
            db.Platforms.Add(new PlatformInfo { Id = "playstation3", DisplayName = "PlayStation 3", Aliases = ["ps3"], Extensions = [".iso", ".pkg"], ScreenScraperSystemId = ScreenScraperSystemId.PlayStation3 });
            ...
            db.Platforms.Add(new PlatformInfo { Id = "pcfx", DisplayName = "PC-FX", Aliases = ["pcfx", "pc-fx"], Extensions = [".ccd",".chd",".cue",".m3u.","toc.zip"], ScreenScraperSystemId = ScreenScraperSystemId.PCFX });
            db.Platforms.Add(new PlatformInfo { Id = "supergrafx", DisplayName = "PC Engine SuperGrafx", Aliases = ["supergrafx"], Extensions = [".ccd",".chd",".cue",".pce",".rom",".sgx",".zip"], ScreenScraperSystemId = ScreenScraperSystemId.PCEngineSuperGrafx });
            ...
            // Other
            db.Platforms.Add(new PlatformInfo { Id = "atomiswave", DisplayName = "Atomiswave", Aliases = ["atomiswave"], Extensions = [".bin", ".dat", ".elf", ".lst", ".zip"], ScreenScraperSystemId = ScreenScraperSystemId.Atomiswave });
            ...
            db.Platforms.Add(new PlatformInfo { Id = "rpgmakerxp", DisplayName = "RPG Maker XP", Aliases = ["rpgmakerxp"], Extensions = [".xp"], ScreenScraperSystemId = ScreenScraperSystemId.EasyRPG });
