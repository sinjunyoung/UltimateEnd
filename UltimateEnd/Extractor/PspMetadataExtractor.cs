        ...
        private static async Task<ExtractedMetadata> ExtractFromCSO(string csoPath)
        {
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    int searchBlocks = Math.Min(10000, (int)numBlocks);
                    ...
                    return ExtractMetadataFromStream(decompressed);
                }
                catch
                {
                    return null;
        ...
        private static byte[] DecompressZlib(byte[] compressed, int expectedSize)
        {
            try
            {
                ...
                while (totalRead < expectedSize &&
                       (bytesRead = deflateStream.Read(decompressed, totalRead, expectedSize - totalRead)) > 0)
                {
                    totalRead += bytesRead;
                }
